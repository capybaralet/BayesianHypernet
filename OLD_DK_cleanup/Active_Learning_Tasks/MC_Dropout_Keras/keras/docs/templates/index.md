...
- __Modularity.__ A model is understood as a sequence or a graph of standalone, fully-configurable layers that can be plugged together with as little restrictions as possible. In particular, neural layers, cost functions, optimizers, initialization schemes, activation functions, regularization schemes are all standalone layers that you can combine to create new models.
...
- __Easy extensibility.__ New layers are dead simple to add (as new classes and functions), and existing layers provide ample examples. To be able to easily create new layers allows for total expressiveness, making Keras suitable for advanced research.
